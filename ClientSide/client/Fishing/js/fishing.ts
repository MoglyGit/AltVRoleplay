...
let t = 0;
let tc = 0;
alt.on('addFish', (amoun: number, type: number, typeChance: number) => {
...
function HittetFish() {
    ...
    for (let i = 0; i < fishes.length; i++) {
        ...
        if (getOffset(fishes[i]).top + 30 > getOffset(rope).top && getOffset(rope).top > getOffset(fishes[i]).top) {
            if (
                getOffset(fishes[i]).left + 30 > getOffset(rope).left &&
                getOffset(rope).left > getOffset(fishes[i]).left
            ) {
...
function getFishType(type: number) {
