...
import * as inv from '../inventory/typescript';
...
let interactionVehicle: alt.Vehicle = null;
let isInVehicleCheck: boolean = false;
...
function ShowVehicleNumberPlate(tuev: string, numberplate: string, name: string) {
...
function ShowInteractMenu(haskey, veh: alt.Vehicle, isMechanic = false, isTuner = false) {
    if (view == null) {
        interactionVehicle = veh;
        view = new alt.WebView(url);

        const player = alt.Player.local;
        const canUseTrunkHoodHandschufach = !(
            native.isThisModelABike(interactionVehicle.model) || native.isThisModelABicycle(interactionVehicle.model)
        );
        ...
        } else if (player.vehicle != null && player.seat == 2)
            view.emit('ShowinVehicleMenuCoDriver', canUseTrunkHoodHandschufach);
        if (haskey) view.emit('ShowLockMenu', interactionVehicle.lockState == 2);

        if (player.vehicle == null) {
            if (interactionVehicle.wheelsCount > 0) {
                const pos: alt.Vector3 = math.getEntityRearPosition(interactionVehicle.scriptID);
        ...
        if (
            ...
            native.getVehicleClass(interactionVehicle) == 18
        )
            view.emit('ShowSirenToggle', veh.hasSyncedMeta('siren'), veh.hasSyncedMeta('shortsiren'));
        if (player.vehicle != null) {
...
function ToggleGurt() {
    if (!alt.hasMeta('GurtUsed')) {
        ...
        let text: string = alt.getMeta('GurtUsed') == false ? 'Angeschnallt' : 'Abgeschnallt';
...
function toggleFront() {
    if (interactionVehicle != null)
        alt.emitServer('TryToggleHood', interactionVehicle, interactionVehicle.seatCount <= 2);
}

function toggleTrunk() {
    if (interactionVehicle != null)
        alt.emitServer('TryToggleTrunk', interactionVehicle, interactionVehicle.seatCount <= 2);
}
...
function handleControls() {
    ...
    if (interactionVehicle != null && !isInVehicleCheck && alt.Player.local.pos.distanceTo(interactionVehicle.pos) > 4)
        closeView();
