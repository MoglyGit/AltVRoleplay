function selectOnlyThis(id, number) {
    for (var i = 1; i <= 3; i++) {
        (<HTMLInputElement>document.getElementById(number + 'Check' + i)).checked = false;
    }
...
alt.on('setText', (text, money, eat, product) => {
    document.getElementById('info').innerHTML =
        text +
        ' Menü<br>Nahrungs Produkte: ' +
        eat +
        '<br>Sonstige Produkte: ' +
        product +
        '<br>Business Konto: <font color="#00ff00">' +
        money +
        '$';
});
...
function changeSellItem(ev, number) {
    ev.preventDefault();
    const price = parseInt((<HTMLInputElement>document.getElementById(number + 'SellPrice')).value);
    if (price < 0 || price > 10000) {
        document.getElementById(number + 'SellInfoitem').innerHTML =
            "<font color='red'>[Warnung]</font> Preis nicht akzeptiert";
        return;
    }
    let item = null;
    for (var i = 1; i <= 4; i++) {
        if (!document.getElementById(number + 'Check' + i)) continue;
        if ((<HTMLInputElement>document.getElementById(number + 'Check' + i)).checked) {
            item = (<HTMLInputElement>document.getElementById(number + 'Check' + i)).value;
        }
    }
    if (item == null) {
        document.getElementById(number + 'SellInfoitem').innerHTML =
            "<font color='red'>[Warnung]</font> Kein Produkt ausgewählt";
        return;
    }
