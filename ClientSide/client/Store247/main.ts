...
const url = `http://resource/client/Store247/Store247Buy.html`;
let view = null;
let cameraControlsInterval = null;
let price = 0;
let dbid = 0;
alt.onServer('ShowStore247Buy', ShowStore247Buy);

function ShowStore247Buy(name: string, m: number, id: number) {
    if (view == null) {
        view = new alt.WebView(url);
        view.emit('setText', name, m);
        view.on('closeView', closeView);
        view.on('buy247Shop', buy247Shop);
...
function buy247Shop() {
    if (dbid != 0) alt.emitServer('Buy247Shop', price, dbid);
    closeView();
}
