...
function changeGehalt(ev) {
    ...
    const gehalt = parseInt((<HTMLInputElement>document.getElementById('newgehalt')).value);
    if (gehalt <= 0) return;
    alt.emit('changegehalt', parseInt(actuallWorker.dataset.scid), gehalt);
    actuallWorker.innerHTML =
        'Name: ' +
        actuallWorker.dataset.name +
        '<br>Rang: ' +
        getFrimenRankNames(actuallWorker.dataset.rank) +
        '<br>Gehalt: ' +
        gehalt;
}
...
function changeRank(rank) {
    ...
    actuallWorker.innerHTML =
        ...
        actuallWorker.dataset.name +
        '<br>Rang: ' +
        getFrimenRankNames(rank) +
        '<br>Gehalt: ' +
        actuallWorker.dataset.gehalt;
}
...
alt.on('addHudWorker', (scid: number, name: string, gehalt: number, rank: number) => {
