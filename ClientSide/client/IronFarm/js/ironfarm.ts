...
document.body.addEventListener('keydown', (event) => {
    ...
    if (event.keyCode == 32) {
        if (hit != 1 && hitTry != 1) {
            ...
            if (
                document.getElementById('hit').offsetLeft < actualHit.offsetLeft &&
                document.getElementById('hit').offsetLeft + document.getElementById('hit').offsetWidth >
                    actualHit.offsetLeft + 14
            ) {
                ...
                loose -= 1;*/
                document.getElementById('hit').style.backgroundColor = '#FF00009F';
            }
...
alt.on('setStrong', (strong: number) => {
