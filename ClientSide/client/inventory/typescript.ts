...
//import * as inventory from './js/javascript';
...
let view = null;
let otherInfType: number = 0;
let vehicleFromTrunk: alt.Vehicle = null;
let cameraControlsInterval: number;
let checkTrunkInterval: number = null;
let checkTargetPlayerInterval: number = null;
let checkHandschufachInterval: number = null;
let otherBackPack = null;
let blockKeyI = false;

export function RemoveHudItem(id: number) {
    removeHudItem(id);
    if (otherBackPack == id) deleteOtherBackpack();
}
export function AddHudItem(id: number) {
    console.log('Test');
...
export function ShowVehicleTrunk(vehicle: alt.Vehicle) {
...
function getNearPlayer(action: number) {
...
function addOtherHud(size: string, weight: string) {
...
function setWeight(weight: string, max: number) {
    if (view != null) view.emit('setWeight', weight, max);
}
function updateWeigth(playerweight: string, backpackweight: string) {
    if (view != null) view.emit('updateWeigth', playerweight, backpackweight);
}
function updateWeigthOtherP(otherWeight: string, playerweight: string) {
    if (view != null) view.emit('updateWeigthOP', otherWeight, playerweight);
}
function updateWeigthOtherB(otherWeight: string, backpackweight: string) {
...
function checkTrunkUsable() {
    ...
    if (vehicleFromTrunk.wheelsCount > 0) {
        const pos: alt.Vector3 = math.getEntityRearPosition(vehicleFromTrunk.scriptID);
...
function backpackItems(id: number) {
...
function splitItem(itemid: number, amount: number, slot: string) {
...
function openOther(type: number, slolts: number) {
    if (view == null) return;
...
function showInv(state: boolean) {
    if (state) {
...
function openInv(otherType: number, other: number) {
    if (view == null) {
...
function blockInv(state: boolean) {
    blockKeyI = state;
}
