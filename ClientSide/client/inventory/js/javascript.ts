...
let actuallWeight: number = 0;

let backpackMaxWeight: number = 0;
let backpackactuallWeight: number = 0;

let otherinfMaxWeight: number = 0;
let otherinfactualWeight: number = 0;
let otherinfName = '';
...
function splitItemAmount() {
    ...
    var amount = parseInt((<HTMLInputElement>document.getElementById('splitamount')).value);
...
function renameItemServer(ev) {
    ...
    var newItemName = (<HTMLInputElement>document.getElementById('renameItem')).value;
...
function mouseenter(slot) {
    ...
    if (slot.hasChildNodes()) {
        ...
        if (
            parseInt(itemSelected.dataset.ammount) + parseInt(slot.lastChild.dataset.ammount) >
            parseInt(itemSelected.dataset.maxamount)
        )
            return;
    ...
    if (
        !itemStardInv.includes('inv') &&
        itemSelected.dataset.place != '1' &&
        actuallWeight + parseFloat(itemSelected.dataset.mass) > maxWeight
    )
        return;
    ...
    if (stackItem != null) {
        ...
        stackItem.dataset.mass = (parseFloat(stackItem.dataset.mass) - parseFloat(itemSelected.dataset.mass)).toFixed(
            2
        );
        updateAmount(stackItem);
        stackItem = null;
...
function backpackenter(slot) {
    ...
    if (slot.hasChildNodes()) {
        ...
        if (
            ...
            return;
        if (
            !itemStardInv.includes('back') &&
            backpackactuallWeight + parseFloat(itemSelected.dataset.mass) > backpackMaxWeight
        )
            return;
        if (stackItem == null) addStack(slot.lastChild);
        return;
...
function otherenter(slot) {
    ...
    if (slot.hasChildNodes()) {
        ...
        if (
            ...
            return;
        if (
            !itemStardInv.includes('other') &&
            otherinfactualWeight + parseFloat(itemSelected.dataset.mass) > otherinfMaxWeight
        )
            return;
        if (stackItem == null) addStack(slot.lastChild);
        return;
    ...
    if (itemSelected != null) {
        ...
        if ('ground' in itemSelected.dataset && itemSelected.dataset.place != 0) {
            if (stackItem != null) {
                alt.emit(
                    'stackInvItem',
                    parseInt(stackItem.dataset.dbid),
                    parseInt(stackItem.dataset.ammount),
                    parseInt(itemSelected.dataset.dbid),
                    true,
                    stackItem.parentNode.id,
                    itemStardInv
                );
                stackItem.style.backgroundColor = '#00000000';
                stackItem = null;
        ...
        if (itemStardInv != itemSelected.parentNode.id && itemSelected.dataset.place != 0)
            alt.emit(
                'itemmoveinv',
                parseInt(itemSelected.dataset.dbid),
                itemStardInv,
...
function updateOtherInfMass() {
    document.getElementById('interactionInvInfo').innerHTML =
        otherinfName + ' (' + otherinfactualWeight.toFixed(2) + '/' + otherinfMaxWeight + 'kg)';
}
function updateBackWeightInfo() {
    ...
    document.getElementById('backinf3').innerHTML =
        'Rucksack (' + backpackactuallWeight.toFixed(2) + '/' + backpackMaxWeight + 'kg)';
...
function createitem(id: number, des: string, type: number, ammount: number, mass: number, maxamount: number) {
...
alt.on('addBackHud', (size, weight: string) => {
    if (hasbackpack == 0) {
        removebackpack();
        return;
    }
...
alt.on('addOtherHud', (size: string, weight: string) => {
...
alt.on('setWeight', (weight: string, max: number) => {
    maxWeight = max;
...
function showItemInfo(item) {
    ...
    if (item.dataset.type == 1) {
        ...
        itemString =
            ...
            item.dataset.mass +
            ' (' +
            item.dataset.ammount +
            '/' +
            item.dataset.maxamount +
            ')';
    }
    document.getElementById('itemName').innerHTML = itemString;
}

alt.on('updateWeigth', (w: string, b: string) => {
...
alt.on('updateWeigthOP', (w: string, b: string) => {
    const mass = parseFloat(w.replace(',', '.'));
    const backmass = parseFloat(b.replace(',', '.'));
...
alt.on('updateWeigthOB', (w: string, b: string) => {
    const mass = parseFloat(w.replace(',', '.'));
    const backmass = parseFloat(b.replace(',', '.'));
    otherinfactualWeight += mass;
    backpackactuallWeight += backmass;
...
alt.on('updateSplitItemAmount', (amount: number, mass: number) => {
    if (splitItem == null) return;
...
alt.on('UpdateItemAmount', (id: number, amount: number, mass: number) => {
...
alt.on('setOtherText', (text: string) => {
    document.getElementById('interactionInvInfo').innerHTML = text;
    otherinfName = text;
});

alt.on('createOtherInv', (slots: number) => {
