...
import * as inventory from '../inventory/typescript.js';
...
alt.onServer('entitySync:updateData', (entityId, entityType, newEntityData)=>{
    ...
    if (entityType == 1) {
        ...
        native.setEntityRotation(objects[entityId], parseFloat(currentEntityData['roll']), parseFloat(currentEntityData['pitch']), parseFloat(currentEntityData['yaw']), 1, true);
    }
});
...
async function groundItems(entityId, currentEntityData, position) {
    ...
    const groundZ = native.getGroundZFor3dCoord(position.x, position.y, position.z + 5, undefined, undefined, undefined);
    if (groundZ[1] != null && groundZ[1] - position.z < 5) objects[entityId] = native.createObject(currentEntityData['obj'], position.x, position.y, groundZ[1] + 0.1, false, false, false);
    else objects[entityId] = native.createObject(currentEntityData['obj'], position.x, position.y, position.z - 1, false, false, false);
...
async function createObject(entityId, currentEntityData, position) {
    await alt.Utils.requestModel(currentEntityData['obj']);
    const groundZ = native.getGroundZFor3dCoord(position.x, position.y, position.z + 100, undefined, undefined, undefined);
    if (groundZ[1] != null && currentEntityData['onground']) {
        objects[entityId] = native.createObject(currentEntityData['obj'], position.x, position.y, groundZ[1] + 0.1, false, false, false);
        alt.log('Test1');
    } else {
        objects[entityId] = native.createObject(currentEntityData['obj'], position.x, position.y, position.z, false, false, false);
        alt.log('Test2');
    }
    native.setEntityRotation(objects[entityId], currentEntityData['pitch'], currentEntityData['roll'], currentEntityData['yaw'], 1, true);
    native.freezeEntityPosition(objects[entityId], true);
}
//damage check
alt.everyTick(()=>{
