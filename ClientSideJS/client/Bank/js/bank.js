...
alt.on('hasKonto', (name, money, iba, atm)=>{
...
alt.on('new', ()=>{
    document.getElementById('info').innerHTML = 'Guten Tag, wie ich sehe haben Sie noch kein Konto bei uns. Möchten Sie ein Konto bei uns eröffnen?<br> Unser <b>Angebot</b>:<br>Zinsen: 1.5%(Jeden Payday gratis Geschenkt)<br>Willkommends Bonus: 500$<br>Kredit bis: 5000$<br>Kredit Zinssatz: 20%<br>ATM Extrakosten: 10%(Anfallende Kosten bei ATM nutzung)<br>';
});
...
function ueberweisung(ev) {
    ev.preventDefault();
    const m = parseInt(document.getElementById('ueberbetrag').value);
    const e = document.getElementById('ueberid').value;
    const r = document.getElementById('ueberrs').value;
...
function konto() {
    ...
    if (mon > 0) {
        document.getElementById('info').innerHTML = 'Willkommen ' + na + '<br>IBAN: ' + iban + "<br>Kontostand: <font color='#00ff00'><b>" + value + '</b></font>';
    } else {
        document.getElementById('info').innerHTML = 'Willkommen ' + na + '<br>IBAN: ' + iban + "<br>Kontostand: <font color='red'><b>" + value + '</b></font>';
    }
}
function einzahlen() {
    const money = parseInt(document.getElementById('betrag').value);
    if (money > 0) {
