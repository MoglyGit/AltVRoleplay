...
function ShowTunerMenu(veh) {
    if (view == null && veh != null) {
        ...
        view.emit('updateTire', native.getVehicleWheelType(cloneVehicle), native.getVehicleMod(cloneVehicle, 23), tireColor[2]);
...
function changeTuning() {
    ...
    const tireColor = native.getVehicleExtraColours(cloneVehicle);
    alt.emitServer('SyncTuning', cloneVehicle, JSON.stringify(tuningParts), JSON.stringify(primCol), JSON.stringify(secCol), native.getVehicleNeonEnabled(cloneVehicle, 0), JSON.stringify(neoncolor), native.getVehicleWheelType(cloneVehicle), tireColor[2], nos, chip);
    alt.log(tuningParts);
    usedsynced = true;
}
