...
function changeTireType(type) {
    ...
    document.getElementById('tirevalue').value = '0';
}
...
alt.on('setExtrasLoad', (type, value)=>{
    if (type != 13) addExtra(type, '' + document.getElementById('ex' + type).value);
    else {
...
alt.on('updateTire', (type, val, col)=>{
    let tireName = getTireTypeName(type);
    document.getElementById('tireinfo').innerHTML = 'Reifen Type: ' + tireName;
    document.getElementById('tireType').value = type;
    document.getElementById('tirevalue').value = val;
    document.getElementById('tirecol').value = col;
});
alt.on('setMaxTires', (max)=>{
    document.getElementById('tirevalue').max = max;
});
