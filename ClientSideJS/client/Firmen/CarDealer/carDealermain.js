...
import * as cam from './carDealerCam.js';
...
function buyVehicle() {
    ...
    alt.emitServer('CarDealerBuyVehicle', native.getEntityModel(cloneVehicle), colorP[1], colorP[2], colorP[3], colorS[1], colorS[2], colorS[3]);
    closeView();
}
function setInText(state) {
    if (state == 1) {
        allowKeys = false;
    } else {
        allowKeys = true;
...
async function changeVehicleFroMServerSide(allow = false, modelname = '', price = 0, tanktype = 0, tank = 0, koff = 0) {
    if (!allow) {
    ...
    if (view != null) {
        ...
        view.emit('setVehicleInfos', modelname, price, tanktype, tank, koff, native.getVehicleModelEstimatedMaxSpeed(modelhash) * 3.6);
    }
}
