...
function ueberweisungFB(ev) {
    ...
    if (!/^[A-Za-z!,.? ]*$/.test(r)) {
        document.getElementById('info2').innerHTML = "<font color='red'>[Warnung]</font> Nicht erlaube zeichen im Grund";
        return;
    }
...
alt.on('setText', (text, konto)=>{
    ...
    if (konto > 0) {
        document.getElementById('info').innerHTML = '<b>IBAN: ' + text + "<br>Kontostand: <font color='#00ff00'><b>" + value + '</b></font>';
    } else {
        document.getElementById('info').innerHTML = '<b>IBAN: ' + text + "<br>Kontostand: <font color='red'><b>" + value + '</b></font>';
    }
});
