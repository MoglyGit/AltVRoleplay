...
export function SetAllowDraw(state) {
    allowDraw = state;
}
alt.everyTick(()=>{
...
alt.on('keydown', (key)=>{
    if (key == 69 && alt.gameControlsEnabled()) {
        ...
        for(let i = 0; i < entity.textLabels.length; i++){
            ...
            const dist = Math.sqrt(Math.pow(playerPos.x - entity.textLabels[i][1], 2) + Math.pow(playerPos.y - entity.textLabels[i][2], 2));
            console.log('distance text:' + dist);
            if (dist > entity.textLabels[i][5]) continue;
            const pos = new alt.Vector3(entity.textLabels[i][1], entity.textLabels[i][2], entity.textLabels[i][3]);
...
function checkmuell() {
    ...
    for(let i in muell){
        ...
        if (e) {
            ...
            native.taskPlayAnim(player, 'anim@gangops@facility@servers@bodysearch@', 'player_search', 1, 1, 1000, 0, 0, false, false, false);
            return;
        }
    }
...
function load3DText() {
    ...
    for(let i = 0; i < test.length; i++){
        ...
        for(let x in muell){
            ...
            drawText3d('Mülltonne durchsuchen', test[i].pos.x, test[i].pos.y, test[i].pos.z + 1, 0.4, 4, 255, 255, 255, 255);
            break;
        }
        for(let x in muellcane){
            if (test[i].model != muellcane[x]) continue;
            drawText3d('Mülleimer durchsuchen', test[i].pos.x, test[i].pos.y, test[i].pos.z + 1, 0.4, 4, 255, 255, 255, 255);
            continue;
        }
        if (test[i].model == alt.hash('prop_atm_03') || test[i].model == alt.hash('prop_atm_02') || test[i].model == alt.hash('prop_atm_01') || test[i].model == alt.hash('prop_fleeca_atm')) {
    ...
    if (alt.Player.local.hasSyncedMeta('Aduty')) {
        ...
        obj.forEach((element)=>{
            ...
            drawText3d('Objecthash: ' + element.model, element.pos.x, element.pos.y, element.pos.z, 0.4, 4, 255, 255, 255, 255);
        });
    }
    for(let i = 0; i < entity.textLabels.length; i++){
        if (!Array.isArray(entity.textLabels)) continue;
        if (!Array.isArray(entity.textLabels[i])) continue;
        if (!entity.textLabels[i][0] || entity.textLabels[i][0] == '') continue;
        drawText3d(entity.textLabels[i][0], entity.textLabels[i][1], entity.textLabels[i][2], entity.textLabels[i][3], 0.4, 4, 255, 255, 255, 255);
    }
    const players = [
        ...alt.Player.all
    ];
    for(let i = 0; i < players.length; i++){
        ...
        if (!friends.IsSocialClubIdFriend(scid) || native.getPedDrawableVariation(player, 1) > 0) {
            ...
            drawText3d('Unbekannter (' + id + ')', player.pos.x, player.pos.y, player.pos.z + 1, 0.4, 4, 255, 255, 255, 255);
            continue;
        }
...
export function drawText2d(msg, x, y, scale, fontType, r, g, b, a, useOutline = true, useDropShadow = true, layer = 0, align = 0) {
    let hex = msg.match('{.*}');
    if (hex) {
        const rgb = hexToRgb(hex[0].replace('{', '').replace('}', ''));
        r = rgb[0];
